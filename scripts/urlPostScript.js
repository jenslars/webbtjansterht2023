...
urlInput_1.addEventListener("input", function () {
    ...
    else if(validateYouTubeVideoTimestampUrl(inputValue)) {
        submitButton.style.pointerEvents = "auto";
        console.log("Valid YouTube URL");
        submitButton.classList.add("active");
        errorMessage.innerText = "";
...
function validateYouTubeVideoTimestampUrl(urlToParse) {
    if (urlToParse) {
        // Updated regular expression to ensure it matches only the URL without extra text after it
        var regExp = /^(https?:\/\/)?(www\.)?(youtube\.com\/watch\?v=|youtu\.be\/)[\w-]+(&t=\d+m?\d*s?)?$/;
        // Use match to extract valid URL part up to the optional timestamp
        var match = urlToParse.match(regExp);
        if (match) {
            // Check if the matched URL is exactly the same as the input, indicating no extra text
            return match[0] === urlToParse;
        }
    }
    return false;
}
...
urlInput_3.addEventListener("input", function () {
    ...
    if (validateYouTubePlaylistUrl(inputValue)) {
        console.log("Valid YouTube URL");
        submitButton3.style.pointerEvents = "auto";
        ...
        submitButton3.style.pointerEvents = "none";
    }
});
...
function addToPlaylist(selectedPlaylistId, selectedTrackUris) {
    ...
    fetch("http://localhost:5000/addTracksToPlaylist", {
        method: "POST", headers: {
            ...
            playlistId: selectedPlaylistId, trackUris: selectedTrackUris,
        }),
    })
...
function convertPlaylist() {
    var errorElementId = 'error-message3';
    ...
    var url = document.getElementById("convertPlaylistInput");
    url.disabled = true;
    submitButton3.classList.remove("active");
    submitButton3.disabled = true;
    fetch("/convertPlaylist?url=" + encodeURIComponent(url.value), {
        method: "GET",
    })
        .then((response) => {
            if (response.status === 400) {

                throw new Error("No playlist found with the provided URL or error in processing.");
            }
            if (!response.ok) {
        ...
        .catch((error) => {
            console.error("Error:", error);
            displayErrorMessage(error.message, 'error-message3');
...
function identifyAllSongs() {
    var url = document.getElementById("identifyAllSongsInput");
    var errorElementId = 'error-message2';
    var submitButton2 = document.getElementById("URLsubmit-btn2");

    console.log("In identify all songs");
    addLoaderToButton("URLsubmit-btn2"); // This adds a loading indication to the button
    clearErrorMessage(errorElementId); // Clear any existing error message
    submitButton2.style.pointerEvents = "none";
    url.disabled = true;
    submitButton2.disabled = true;
    submitButton2.classList.remove("active");
    fetch("/identifyAllSongsInVideo?url=" + encodeURIComponent(url.value), {
        method: "GET",
    })
        .then((response) => {
            if (response.status === 400) {
                // Handle 400 specifically, indicating no song found or error in conversion
                throw new Error("No songs found with the provided URL or error in processing the video.");
            }
            if (!response.ok) {
                // General error handling for other server-side errors
                throw new Error("Internal server error");
            }
            return response.json();
        })
        .then((data) => {
            console.log("Backend response:", data);
            if (data.tracks && Array.isArray(data.tracks)) {
                // Process and display the tracks if the array is valid
                selectedTrackUris = data.tracks
                    .slice(0, 50) // Limit to first 50 tracks
                    .map((track) => track.uri) // Extract the URIs
                    .filter((uri) => uri); // Filter out any undefined or null URIs
                var serviceContainer = document.getElementById("serviceContainer");
                var expandedIdentifyAllSongsContainer = document.getElementById("expandedIdentifyAllSongsContainer");
                ...
                // Log an error if the data.tracks is not a valid array
                console.error("Data.tracks is not an array");
            }
        ...
        .finally(() => {
            removeLoaderFromButton("URLsubmit-btn2"); // This function should remove the loading indication
            submitButton2.style.pointerEvents = "auto"; // Make the button clickable again
            url.disabled = false;
...
function convertVideo() {
    var errorElementId = 'error-message1';
    var submitButton1 = document.getElementById("URLsubmit-btn1");
    console.log("In convert video");
    addLoaderToButton("URLsubmit-btn1");
    submitButton1.style.pointerEvents = "none";
    clearErrorMessage(errorElementId);
    var url = document.getElementById("convertVideoInput");
    submitButton1.disabled = true;
    url.disabled=true;
    submitButton1.classList.remove("active");
    fetch("/convertVideo?url=" + encodeURIComponent(url.value), {
        method: "GET",
    })
        .then((response) => {
            url.disabled = true;
        ...
        .finally(() => {
            removeLoaderFromButton("URLsubmit-btn1"); // This function should remove the loading indication
            submitButton1.style.pointerEvents = "auto"; // Make the button clickable again
            url.disabled = false;
...
function displayErrorMessage(message, errorElementId) {
    var errorMessageElement = document.getElementById(errorElementId);
    if (errorMessageElement) {
        errorMessageElement.textContent = message; // Set the text content to the error message
